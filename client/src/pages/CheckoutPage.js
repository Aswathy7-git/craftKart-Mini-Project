...
const CheckoutPage = () => {
  ...
  // Match server: free shipping over ₹10,000; otherwise ₹99
  const shipping = subtotal > 10000 ? 0 : 99;
  // Match server: 18% GST on subtotal
  ...
  const isStepValid = (step) => {
    switch (step) {
      ...
      case 2:
        return !!formData.paymentMethod; // PayU/COD only
      default:
        return true;
    }
  };
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-4">
                        <p className="font-medium capitalize">
                          {formData.paymentMethod === 'payu' ? 'PayU UPI' : (formData.paymentMethod === 'cod' ? 'Cash on Delivery' : formData.paymentMethod)}
                        </p>
                      </div>
                    </div>
                    ...
                    {giftItems.map((item) => (
                      <div key={item._id} className="flex items-center space-x-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500">Qty: {item.quantity}</p>
                          {item.recipientName && (
                            <p className="text-xs text-primary-600">For: {item.recipientName}</p>
                          )}
                        </div>
                        <p className="text-sm font-medium">₹{(item.price * item.quantity).toFixed(2)}</p>
                      </div>
